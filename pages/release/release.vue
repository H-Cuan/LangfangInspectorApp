<template>
	<view>
				...
				<view class="form">
					<view style="display: flex; align-items: center;height: 40px;justify-content: space-between;">
						...
						<view style="display: flex;align-items: center;">
							...
							<image src="/static/right.png" style="height: 15px;width: 15px;margin-left: 10px;" alt="">
						</view>
					...
					<view style="display: flex;align-items: center;height: 40px;justify-content: space-between;">
						<view>督察方式：</view>
						<view style="display: flex;align-items: center;">
							<picker @change="bindPickerChange2($event,range2)" :value="index2" :range="range2"
								:range-key="'label'">
								<view class="uni-input">{{range2[index2].label}}</view>
							</picker>
							<image src="/static/right.png" style="height: 15px;width: 15px;margin-left: 10px;" alt="">
						</view>
					...
					<view style="display: flex;align-items: center;height: 40px;justify-content: space-between;">
						<view>督察内容：</view>
						<view style="display: flex;align-items: center;">
							<picker @change="bindPickerChange3($event,range3)" :value="index3" :range="range3"
								:range-key="'super_content'">
								<view class="uni-input">{{range3[index3].super_content}}</view>
							</picker>
							<image src="/static/right.png" style="height: 15px;width: 15px;margin-left: 10px;" alt="">
						</view>
		...
		<view style="margin-top: 20px;display: flex;justify-content: center;">
			<view style="width: 90%;margin-left: 20px;font-size: 18px;font-weight: bold;">创建分组</view>
		</view>
		<view style="display: flex;margin-top: 5px; justify-content: center;flex-wrap: wrap;">
			<view v-for="(item, index) in form.addGroup" :key="index"
				style="border-radius: 3%;width: 90%;background-color: #fff;">
				<view class="form">
					<input type="text" placeholder="请输入小组名称" v-model="item.group_name">
				</view>
				<view style="margin: 10px;display: flex;">
					<view v-if="show" style="margin-right: 10px;" v-for="(user, index) in item.addMember" :key="index">
						<image style="height: 45px;width: 45px;border-radius: 10px;" :src="user.avatar">
							<image @click='deleteUser(item,index)' src="/static/shanchu.png"
					...
					<image @click="addUser($event,index)" style="height: 45px;width: 45px;"
						src="/static/23f6b108bfb64da1ff5d72202d815d9.png" alt="">

				</view>
				<view class="form" style="display: flex;">
					<text style="font-size: 18px;font-weight: bold;">任务描述：</text>
					<input type="text" placeholder="请输入任务描述" v-model="item.task_desc">
				</view>
				<view style="margin-left: 80%;"><button v-if="index !== 0" style="background-color: red;" size="mini"
						@click="deleteItem(item, index)">删除</button></view>
				<view style="display: flex;justify-content: center;">
					<view style="border-top: 1px #ccc solid;width: 90%;"></view>
				</view>
				<view style="margin-top: 10px;display: flex;justify-content: center;width: 100%;">
					<button v-if="index+1 == form.addGroup.length" @click="addItem(form.addGroup.length)"
						style="height: 40px; margin-bottom: 10px; border:1px #007aff solid;width: 90%;color: #007aff;border-radius: 10px;font-size: 25px;display: flex;justify-content: center;align-items: center;">+</button>
				</view>
			</view>
		</view>
